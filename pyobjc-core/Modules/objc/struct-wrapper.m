...
static PyObject* _Nullable struct_mp_subscript(PyObject* self, PyObject* item)
{
    ...
    if (PyIndex_Check(item)) {
        ...
        if (PySlice_GetIndicesEx(SLICE_CAST(item),STRUCT_LENGTH(self), &start, &stop, &step,
                                 &slicelength)
            < 0) {
            return NULL;
        }
            ...
            void* _Nullable* _Nonnull cargs, void* _Nonnull userdata)
{
    ...
    if (kwds != NULL) {
        ...
        while (PyDict_Next(kwds, &pos, &key, &value)) {
            ...
            PyObject* temp = PyUnicode_AsUTF8String(key);
            if (temp == NULL) {
                return;
            }
            const char* k_bytes = PyString_AsString(temp);
            Py_DECREF(temp);
